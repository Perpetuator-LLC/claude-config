...
- **Every command is self-contained + runnable + verified against live state** — the `🔑 PROMPTS` line (each prompt's machine + bao role + namespace, in run order) goes ABOVE the fence next to the `► <machine>` header, `cd <dir>` as the block's first line. NO `<placeholders>` for any value (discover via command substitution; secrets via `read -rs` in-block — but prefer store-to-store copies the human never sees). **SSH in hand-overs uses the tailnet ALIASES that match the ansible inventory hostnames (`ssh cc-stage`, `ssh cc-prod`, `ssh lestrange`) — never raw IPs, never public addresses** (2026-07-16 standard; boxes are tailnet-only). If an alias might be missing, prefix the block with `ssh -G <alias> >/dev/null || echo 'missing ~/.ssh/config alias — see mcp docs/ENDPOINTS.md'`. **The `ssh <alias>` line goes in its OWN tiny block (or just the ► header names the machine) — never as line 1 of the command block**, because pasting the combined block runs the commands on the LOCAL machine while ssh waits (2026-07-18). **Multi-command remote blocks must fail-fast**: chain with `&&` (or open with `set -e`) so a failed first command doesn't let the rest run blind. Verify against where things ARE now, and probe a URL/endpoint yourself before handing it over. **Interactive prompts must self-describe IN the tool, not just in the hand-off note**: an ansible run handed to the human uses `vars_prompt` with prompt text naming the exact secret/account/box it wants (e.g. "sudo password for ops@cc-stage") instead of the bare `--ask-become-pass` "BECOME password:"; same for any `read -rs` — the human at the prompt should never have to scroll back to know which credential is being asked for (2026-07-19; pattern proven in cc-alloy.yml per-`--limit` prompt naming). Full hand-off format: `governance/operations.md`.
