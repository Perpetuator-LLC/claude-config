...
- **Role governance** → `~/.claude/governance/<domain>.md` (technical · security · financial · legal · marketing · operations · product). Load the matching domain BEFORE substantive work in it — always `security.md` before any secret/credential/infra-secret work, `technical.md` before infra/dev-flow mechanics, `operations.md` before a non-trivial hand-off.
...
The rest is procedure — load `security.md` when doing that work:
...
- **Offer choices as a numbered table** (a `#` column) so the user can reply with a number; mark exactly one `(Recommended)` with a one-line why.
...
- **Every command is self-contained + runnable + verified against live state** — `► <machine>` header outside the fence, `cd <dir>` as the block's first line, a `🔑 PROMPTS` line naming each prompt's machine + bao role + namespace in run order. NO `<placeholders>` for any value (discover via command substitution; secrets via `read -rs` in-block). Verify against where things ARE now, and probe a URL/endpoint yourself before handing it over. Full hand-off format: `operations.md`.
