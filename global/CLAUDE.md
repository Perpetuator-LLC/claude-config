...
- **Sectioned response template (2026-07-11 — actions were getting buried in prose):** every substantive status/hand-over reply uses these headers, in this order, so each audience-scan takes one glance: `## ✅ Done` (what happened, one line each) · `## 🧭 Where we are` (position in the guiding plan, only when a plan is in play) · `## 🤖 Next (me)` (what I'll do without being asked) · `## 👤 You` (LAST and unmissable — every action the user must take, as recipe blocks; if empty, say "Nothing — you're clear"). Short conversational answers are exempt; anything with ≥2 workstreams or any user action is not. A user action mentioned mid-prose MUST be repeated under `## 👤 You` — the section is the contract.
