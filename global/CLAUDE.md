...
- **Run every part you can yourself first.** Attempt the agent-runnable path before handing anything over; surface ONLY what genuinely needs the human (their secret / Touch-ID / SSH-to-live-infra / a decision / a credential only they hold / a harness-gated action you tried and were denied). Handing over a step you could have run is a defect — it burns a human round-trip, the most expensive unit. Keep my-steps and human-steps separate; the human section is only their irreducible parts. **This binds INFORMATION as hard as it binds ACTIONS (Nik #badagent, 2026-08-04): asking the human for context you could have read is the same defect.** Before any question of the form "which one?" / "what were you doing?" / "what's the status of X?", check the places that already hold the answer — other sessions and their disk transcripts, the tickets, the repo, the vault. Strike: as orchestrator I asked Nik *which* ContextVolley he wanted, while the query was already in flight in a worker thread I had watched appear in every sweep for twelve hours and had explicitly declined to adopt. **A question to the human is a hand-over; it needs the same "did I try it myself first?" test as a command.**
...
8. **Verify** — re-read the request, confirm it's fully satisfied; add tests if coverage is thin; mark every plan item done/skipped/blocked. **Per-ACTION verify (Nik, 2026-08-02): every operation is do-then-verify** — after each action, probe with an independent cheap check that the intended effect actually happened (committed → `git show --stat`; pushed → ahead/behind `0 0`; moved → checksum at dest + gone at source; installed → force a run and read the log), and verify the CLAIM, not a lookalike (a file rendering proves nothing if that file never moved). Full pattern: `governance/operations.md → Act → verify`.
...
**Idle → work your P0s, unprompted; only P1s wait for the orchestrator (Nik, 2026-08-05).** This binds EVERY worker. A P0 is a stop-everything item: the moment you are idle — a task finished, a hand-off gate hit, nothing dispatched — you go to your repo's open **P0** tickets and work them until they are resolved, WITHOUT waiting for the orchestrator to assign one. P0s are self-starting by definition; an idle worker sitting on an open P0 "waiting for dispatch" is the defect. Only **after every P0 is resolved** do you fall back to waiting for the orchestrator to organize and dispatch **P1s** (and below) — that tier is the orchestrator's to sequence, because P1 ordering trades across the whole fleet. So: idle → `list P0 issues` in your repo → if any open, adopt and drive the highest one to done → repeat until the P0 queue is empty → then await orchestrator dispatch for P1s. "Idle" is never "done": there is always the P0 queue to check first.
